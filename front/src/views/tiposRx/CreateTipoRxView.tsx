import { createTipoRx } from "@/api/tipoRxAPI";
...
export default function CreateTipoRxView() {
    ...
    <>
      <div className="mb-6 flex flex-col gap-4 border-b border-secondary-dark/60 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-semibold tracking-tight text-slate-900">Crear tipo de RX</h2>
        </div>
      </div>

      <div className="flex-1">
        <form onSubmit={handleSubmit((formData) => mutation.mutate(formData))} className="w-full space-y-5 rounded-2xl border border-secondary-dark/60 bg-white p-6 shadow-sm">
